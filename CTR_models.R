...
cross_validation_r <- function(
  ...
  for(timecn in 1: 5){
    ...
    if (data_name == "METABRIC"){
      trainingData[[outcome_name]] <- ifelse(trainingData[[outcome_name]] == 1, 0, 1)
    }


    #Training phase
    trainingData <- dplyr::select (trainingData, -remove_attributes)
    # trainingData <- dplyr::select(trainingData, -dplyr::any_of(remove_attributes))
    # trainingData <- trainingData %>%
    #   dplyr::select(-dplyr::any_of(remove_attributes))
...
estimateUpLiftScore_TE<- function(model, outComeColName,training_treatment, estimationType, infileName, outputBaseFolder, exceptAttrbute = c(), threshold){
    input_data <-read.csv(file = infileName)
        # Flip 0 → 1 and 1 → 0
    if (data_name == "METABRIC"){
      input_data[[outcome_name]] <- ifelse(input_data[[outcome_name]] == 1, 0, 1)
    }
  ...
  new_file_name_survival <-   newFileName <- paste(c(secondFileNameParts[1],'_', secondFileNameParts[2], '_', estimationType, '_', threshold, '_', secondFileNameParts[3],
                         '_survival.', fileNameParts[2]), collapse = "")

  full_path_survival <- paste(c(outputdata_name,'/',new_file_name_survival ), collapse = "")
  ...
  # ✅ Save as CSV
  write.csv(test_data_with_rec, full_path_survival, row.names = FALSE)
}
